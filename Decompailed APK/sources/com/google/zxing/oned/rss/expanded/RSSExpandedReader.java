...
public final class RSSExpandedReader extends AbstractRSSReader {
    ...
    List decodeRow2pairs(int i, BitArray bitArray) throws NotFoundException {
        boolean z = false;
        while (!z) {
            try {
        ...
        storeRow(i, false);
        if (!zIsEmpty) {
            List listCheckRows = checkRows(false);
            if (listCheckRows != null) {
                return listCheckRows;
            }
            List listCheckRows2 = checkRows(true);
    ...
    private List checkRows(List list, int i) throws NotFoundException {
        while (i < this.rows.size()) {
            ...
            while (it.hasNext()) {
                this.pairs.addAll(((ExpandedRow) it.next()).getPairs());
            }
    ...
    ExpandedPair retrieveNextPair(BitArray bitArray, List list, int i) throws NotFoundException {
