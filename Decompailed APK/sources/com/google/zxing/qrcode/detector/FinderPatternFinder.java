...
public class FinderPatternFinder {
    ...
    private FinderPattern[] selectBestPatterns() throws NotFoundException {
        ...
        if (size > 3) {
            ...
            float f2 = 0.0f;
            while (it.hasNext()) {
                float estimatedModuleSize2 = ((FinderPattern) it.next()).getEstimatedModuleSize();
                f += estimatedModuleSize2;
                f2 += estimatedModuleSize2 * estimatedModuleSize2;
            }
            float f3 = f / size;
            float fSqrt = (float) Math.sqrt((f2 / r3) - (f3 * f3));
            Collections.sort(this.possibleCenters, new FurthestFromAverageComparator(f3));
            float fMax = Math.max(0.2f * f3, fSqrt);
            int i = 0;
            while (i < this.possibleCenters.size() && this.possibleCenters.size() > 3) {
                if (Math.abs(((FinderPattern) this.possibleCenters.get(i)).getEstimatedModuleSize() - f3) > fMax) {
        ...
        if (this.possibleCenters.size() > 3) {
            Iterator it2 = this.possibleCenters.iterator();
            while (it2.hasNext()) {
                estimatedModuleSize += ((FinderPattern) it2.next()).getEstimatedModuleSize();
            }
