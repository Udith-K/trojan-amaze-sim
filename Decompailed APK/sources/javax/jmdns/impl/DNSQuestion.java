...
public class DNSQuestion extends DNSEntry {
    ...
    private static class Pointer extends DNSQuestion {
        ...
        public void addAnswers(JmDNSImpl jmDNSImpl, Set set) {
            ...
            while (it.hasNext()) {
                addAnswersForServiceInfo(jmDNSImpl, set, (ServiceInfoImpl) ((ServiceInfo) it.next()));
            }
            if (isServicesDiscoveryMetaQuery()) {
                Iterator it2 = jmDNSImpl.getServiceTypes().values().iterator();
                while (it2.hasNext()) {
                    set.add(new DNSRecord.Pointer("_services._dns-sd._udp.local.", DNSRecordClass.CLASS_IN, false, DNSConstants.DNS_TTL, ((JmDNSImpl.ServiceTypeEntry) it2.next()).getType()));
                }
                return;
            }
            if (isReverseLookup()) {
    ...
    /* JADX INFO: renamed from: javax.jmdns.impl.DNSQuestion$1, reason: invalid class name */
