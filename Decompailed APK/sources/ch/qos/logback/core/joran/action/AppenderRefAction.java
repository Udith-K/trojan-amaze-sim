...
public class AppenderRefAction extends Action {
    ...
    public void begin(InterpretationContext interpretationContext, String str, Attributes attributes) {
        ...
        String strSubst = interpretationContext.subst(attributes.getValue("ref"));
        if (OptionHelper.isEmpty(strSubst)) {
            this.inError = true;
            addError("Missing appender ref attribute in <appender-ref> tag.");
            return;
        }
        Appender appender = (Appender) ((HashMap) interpretationContext.getObjectMap().get("APPENDER_BAG")).get(strSubst);
        if (appender != null) {
