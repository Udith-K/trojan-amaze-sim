...
public abstract class TypeUtils {
    ...
    */
    ...
    public static List getImmediateSupertypes(KotlinType kotlinType) {
        ...
        Collection collectionMo2782getSupertypes = kotlinType.getConstructor().mo2782getSupertypes();
        ArrayList arrayList = new ArrayList(collectionMo2782getSupertypes.size());
        Iterator it = collectionMo2782getSupertypes.iterator();
        while (it.hasNext()) {
            KotlinType kotlinTypeCreateSubstitutedSupertype = createSubstitutedSupertype(kotlinType, (KotlinType) it.next(), typeSubstitutorCreate);
    ...
    public static boolean isNullableType(KotlinType kotlinType) {
        ...
        if (constructor instanceof IntersectionTypeConstructor) {
            Iterator it = constructor.mo2782getSupertypes().iterator();
            while (it.hasNext()) {
    ...
    public static boolean hasNullableSuperType(KotlinType kotlinType) {
        ...
        if (kotlinType.getConstructor().mo2781getDeclarationDescriptor() instanceof ClassDescriptor) {
            return false;
        }
        Iterator it = getImmediateSupertypes(kotlinType).iterator();
        while (it.hasNext()) {
            if (isNullableType((KotlinType) it.next())) {
                return true;
            }
    ...
    private static boolean contains(KotlinType kotlinType, Function1 function1, SmartSet smartSet) {
        ...
        TypeConstructor constructor = kotlinType.getConstructor();
        if (constructor instanceof IntersectionTypeConstructor) {
            Iterator it = ((IntersectionTypeConstructor) constructor).mo2782getSupertypes().iterator();
            while (it.hasNext()) {
    ...
    public static TypeParameterDescriptor getTypeParameterDescriptorOrNull(KotlinType kotlinType) {
        ...
        if (kotlinType.getConstructor().mo2781getDeclarationDescriptor() instanceof TypeParameterDescriptor) {
            return (TypeParameterDescriptor) kotlinType.getConstructor().mo2781getDeclarationDescriptor();
        }
        return null;
    }
}
