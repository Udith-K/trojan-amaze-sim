    ...
    public ExternalOverridabilityCondition.Result isOverridable(CallableDescriptor superDescriptor, CallableDescriptor subDescriptor, ClassDescriptor classDescriptor) {
        ...
        if (subDescriptor instanceof JavaMethodDescriptor) {
            ...
            if (typeParameters.isEmpty()) {
                ...
                Sequence sequencePlus = SequencesKt.plus(map, returnType);
                ...
                CallableDescriptor callableDescriptorBuild = (CallableDescriptor) superDescriptor.substitute(new RawSubstitution(null, 1, null).buildSubstitutor());
                ...
                if (callableDescriptorBuild instanceof SimpleFunctionDescriptor) {
                    ...
                    if (!typeParameters2.isEmpty()) {
                        callableDescriptorBuild = simpleFunctionDescriptor.newCopyBuilder().setTypeParameters(CollectionsKt.emptyList()).build();
                        Intrinsics.checkNotNull(callableDescriptorBuild);
                    }
                }
                OverridingUtil.OverrideCompatibilityInfo.Result result = OverridingUtil.DEFAULT.isOverridableByWithoutExternalConditions(callableDescriptorBuild, subDescriptor, false).getResult();
