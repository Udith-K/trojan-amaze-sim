...
public class SubjectPublicKeyInfoChecker {
    ...
    public static void checkInfo(SubjectPublicKeyInfo subjectPublicKeyInfo) {
        ...
        if (X9ObjectIdentifiers.id_ecPublicKey.equals((ASN1Primitive) algorithm)) {
            ...
            if (Primes.hasAnySmallFactors(modulus) || !Primes.isMRProbablePrime(modulus, CryptoServicesRegistrar.getSecureRandom(), getNumberOfIterations(iBitLength, iAsInteger2))) {
    ...
    private static void validate(BigInteger bigInteger) {
        ...
        if (!Primes.enhancedMRProbablePrimeTest(bigInteger, CryptoServicesRegistrar.getSecureRandom(), iBitLength >= 1536 ? 3 : iBitLength >= 1024 ? 4 : iBitLength >= 512 ? 7 : 50).isProvablyComposite()) {
