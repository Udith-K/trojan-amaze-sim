...
public class PasswordRecipientInformation extends RecipientInformation {
    ...
    private PasswordRecipientInfo f75info;
    ...
    PasswordRecipientInformation(PasswordRecipientInfo passwordRecipientInfo, AlgorithmIdentifier algorithmIdentifier, CMSSecureReadable cMSSecureReadable, AuthAttributesProvider authAttributesProvider) {
        ...
        this.f75info = passwordRecipientInfo;
        this.rid = new PasswordRecipientId();
    }

    public String getKeyDerivationAlgOID() {
        if (this.f75info.getKeyDerivationAlgorithm() != null) {
            return this.f75info.getKeyDerivationAlgorithm().getAlgorithm().getId();
        }
        return null;
    }
    ...
    public byte[] getKeyDerivationAlgParams() {
        ...
        try {
            if (this.f75info.getKeyDerivationAlgorithm() == null || (parameters = this.f75info.getKeyDerivationAlgorithm().getParameters()) == null) {
                return null;
            }
    ...
    public AlgorithmIdentifier getKeyDerivationAlgorithm() {
        return this.f75info.getKeyDerivationAlgorithm();
    }

    @Override // org.bouncycastle.cms.RecipientInformation
    protected RecipientOperator getRecipientOperator(Recipient recipient) throws CMSException, IOException {
        PasswordRecipient passwordRecipient = (PasswordRecipient) recipient;
        AlgorithmIdentifier algorithmIdentifier = AlgorithmIdentifier.getInstance(AlgorithmIdentifier.getInstance(this.f75info.getKeyEncryptionAlgorithm()).getParameters());
        return passwordRecipient.getRecipientOperator(algorithmIdentifier, this.messageAlgorithm, passwordRecipient.calculateDerivedKey(passwordRecipient.getPasswordConversionScheme(), getKeyDerivationAlgorithm(), ((Integer) KEYSIZES.get(algorithmIdentifier.getAlgorithm())).intValue()), this.f75info.getEncryptedKey().getOctets());
    }
}
