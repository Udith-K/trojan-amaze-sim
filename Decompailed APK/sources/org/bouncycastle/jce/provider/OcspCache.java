    ...
    static OCSPResponse getOcspResponse(CertID certID, PKIXCertRevocationCheckerParameters pKIXCertRevocationCheckerParameters, URI uri, X509Certificate x509Certificate, List<Extension> list, JcaJceHelper jcaJceHelper) throws CertPathValidatorException {
        ...
        try {
            ...
            for (int i2 = 0; i2 != list.size(); i2++) {
                Extension extensionM = OcspCache$$ExternalSyntheticApiModelOutline0.m(list.get(i2));
                byte[] value = extensionM.getValue();
                if (OCSPObjectIdentifiers.id_pkix_ocsp_nonce.getId().equals(extensionM.getId())) {
                    bArr = value;
                }
                aSN1EncodableVector2.add(new org.bouncycastle.asn1.x509.Extension(new ASN1ObjectIdentifier(extensionM.getId()), extensionM.isCritical(), value));
            }
            try {
